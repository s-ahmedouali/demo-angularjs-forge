package org.funcoding.bbl.rest;
...
import org.funcoding.bbl.model.Event;
...
public class EventEndpoint {
	@PersistenceContext(unitName = "demojs-persistence-unit")
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<Event> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT e FROM Event e LEFT JOIN FETCH e.speakers WHERE e.id = :entityId ORDER BY e.id",
						Event.class);
	...
	public List<Event> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<Event> findAllQuery = em
				.createQuery(
						"SELECT DISTINCT e FROM Event e LEFT JOIN FETCH e.speakers ORDER BY e.id",
						Event.class);
